...
package com.mmr.wordtalk.auth.endpoint;
...
import com.mmr.wordtalk.admin.api.entity.SysTenant;
import com.mmr.wordtalk.admin.api.feign.RemoteClientDetailsService;
import com.mmr.wordtalk.admin.api.feign.RemoteTenantService;
import com.mmr.wordtalk.admin.api.vo.TokenVo;
...
import com.mmr.wordtalk.common.security.util.OAuth2ErrorCodesExpand;
import com.mmr.wordtalk.common.security.util.OAuthClientException;
...
public class WordtalkTokenEndpoint {

	private final WordtalkAuthenticationFailureEventHandler authenticationFailureHandler;
	...
	public R<Page<TokenVo>> tokenList(@RequestBody Map<String, Object> params) {
		...
		List<TokenVo> tokenVoList = redisTemplate.opsForValue().multiGet(pages).stream().map(obj -> {
			...
			WordtalkUser wordtalkUser = (WordtalkUser) authentication.getPrincipal();
			tokenVo.setUserId(wordtalkUser.getId());
			return tokenVo;
		}).filter(tokenVo -> {
			// 根据用户名过滤
